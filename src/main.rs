...
async fn main() {
    ...
    let (globals, mut event_queue) = registry_queue_init::<SimpleWindow>(&conn).unwrap();
    ...
    let handle = tokio::spawn(async move {
        ...
        let fd = AsyncFd::new(backend.poll_fd()).expect("could not create async fd");
        ...
        loop {
            ...
            event_queue
                .dispatch_pending(&mut simple_window)
                .expect("could not dispatch pending");

            let read_guard = event_queue.prepare_read().expect("could not prepare read");

            let guard = fd
                .readable()
                .await
                .expect("could not check if socket is readable");
...
unsafe impl Send for SimpleWindow {}
unsafe impl Sync for SimpleWindow {}
